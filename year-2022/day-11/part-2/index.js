const { ArrayUtils, MathUtils } = require('../../../utils')
...
const makeRound = (monkeys, commonMultiple) => {
	monkeys.forEach(monkey => {
		while (monkey.items.length) {
			monkey.inspections++
			const item = monkey.items.shift()
			const worryLevel = monkey.operation(item) % commonMultiple
...
const contestResponse = input => {
	...
	const commonMultiple = MathUtils.leastCommonMultipleOfList(monkeys.map(x => x.divisibleBy))

	for (let i = 0; i < 10000; i++) {
		makeRound(monkeys, commonMultiple)
	}
