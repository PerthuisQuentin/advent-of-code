...
const searchSizes = (item, result) => {
	const size = calculateSize(item)
	if (size <= SIZE_THRESHOLD) result.push(size)

	item.content
		.filter(subItem => subItem.type === TYPE_DIRECTORY)
		.forEach(subItem => searchSizes(subItem, result))
}
...
const contestResponse = input => {
	...
	const sizes = []
	searchSizes(fileTree, sizes)
	return ArrayUtils.sum(sizes)
}
