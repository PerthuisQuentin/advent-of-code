...
class Droid {
	...
	fillGridWithOxygen() {
		let tick = -1
		this.grid.setCell(this.droidPosition, EMPTY)

		const positionsAnalysed = {}
		let positionsToAnalyse = [this.oxygenPosition]
		...
		while (positionsToAnalyse.length > 0) {
			...
			for (let positionAnalysed of positionsToAnalyse) {
				for (let position of this.grid.getNeighborsPosition(positionAnalysed)) {
					...
					const cell = this.grid.getCell(position)
					if (cell === EMPTY) newPositionsToAnalyse.push(position)
				}

				this.grid.setCell(positionAnalysed, OXYGEN)
			}
...
const contestResponse = input => {
	...
	const ticks = droid.fillGridWithOxygen()
